<template>
    <div>
        <div v-if="judge">
            <div class="first_table_style" id="fourth_charts" @click="dialogVisibleOne = true"></div>
        <!-- **************************************************************************************************************** -->
...
export default({
    ...
    methods:{     
        charts(){
            ...
            this.axios({
                ...
                if(response.data.length != 0){
                    ...
                    this.$echarts.init(document.getElementById('fourth_charts')).setOption({
                        title: {
                        ...
                        radar: {
                            ...
                            indicator: [
                                ...
                                { name: secondName, max: 100 },
                                { name: thirtName, max: 100 },
                                { name: fourthName, max: 100 },
                                { name: fifthName, max: 100 }
                            ]
                        },
                        series: [{
                            ...
                            type: 'radar',
                            data: [{
                                value: [firstData, secondData, thirtData, fourthData, fifthData],
                                name: '掌握程度'
                            }]
                        }]
                    })
        ...
        submitTableData(){
            ...
            this.axios({
                method: 'post',
                url: '/individual/addExpandSkillsData',
                data: postData,
            }).then(response=>{
                if(response.data.length != 0){                
                    this.$message({
        ...
        modifyData(){
            ...
            this.axios({
                method: 'post',
                url: '/individual/modifyExpandSkillsData',
                data: postData,
            }).then(response=>{
                if(response.data.length != 0){                
                    this.$message({
