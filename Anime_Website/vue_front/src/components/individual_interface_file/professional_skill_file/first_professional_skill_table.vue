        ...
        <!-- **************************************************************************************************************** -->
            <el-dialog title="提示" :visible.sync="dialogVisibleOne" width="35%" :before-close="handleClose">
                <div style="font-family: STFangsong; font-size: 25px; font-weight: 500;">
                    ...
                    项目名称六:&emsp;&emsp;<input id="all_input_style" type="text" v-model="modifyTableData[0].sixthItemName" /><br>
                    项目数据六:&emsp;&emsp;<input id="all_input_style" type="text" v-model="modifyTableData[0].sixthItemData" /><br>
                </div>
    ...
    methods:{     
        charts(){
            ...
            this.axios({
                ...
                if(response.data.length != 0){
                    ...
                    var sixth = tableInfor[0].sixthItemName;
                    this.$echarts.init(document.getElementById('first_charts')).setOption({
                        ...
                        xAxis: {
                            type: 'category',
                            data: [first, second, thirt, fourth, fifth, sixth],
                        },
                        yAxis: {
                            type: 'value',
                        },
                        series: [
                            {
                                data: [
                                    ...
                                    {
                                        value: tableInfor[0].sixthItemData,
                                        itemStyle: {
                                            color: '#385466'
                                        }
                                    },
        ...
        modifyData(){
            let postData = this.qs.stringify({
                ...
                sixthItemName: this.modifyTableData[0].sixthItemName,
                sixthItemData: this.modifyTableData[0].sixthItemData,
            });
            this.axios({
                method: 'post',
                url: '/individual/modifyProfessionalSkillData',
                data: postData,
            }).then(response=>{
                if(response.data.length != 0){                
                    this.$message({
